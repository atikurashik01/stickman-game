...
class StickmanJump:
    def __init__(self, master):
        ...
        self.max_jumps = 2
        self.blocks = []
        self.block_speed = 10  # Increased speed for more challenge
        self.block_interval = 40  # Spawn more frequently for more challenge
        self.frame_count = 0
        self.bg_hue = 0
        self.block_hue = 180
        self.score = 0
    ...
    def draw_background(self):
        color = self.hsv_to_hex(self.bg_hue, 0.5, 1)
        self.canvas.create_rectangle(0, 0, self.width, self.height, fill=color, outline="")
        self.bg_hue = (self.bg_hue + 0.2) % 360
        # Draw clouds
        for cloud in self.clouds:
            self.canvas.create_oval(self.offset_x + cloud['x'], self.offset_y + cloud['y'], self.offset_x + cloud['x']+cloud['size'], self.offset_y + cloud['y']+cloud['size']//2, fill='#fff', outline='')
        # Draw ground
        self.canvas.create_rectangle(self.offset_x, self.offset_y + self.ground_y+30, self.offset_x + self.play_width, self.offset_y + self.play_height, fill="#222", outline="")
    ...
    def on_resize(self, event):
        # Center the play area in the window
